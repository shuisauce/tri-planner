...
const DEFAULT_SETTINGS = { annualGoal: 285000, rates: { HFH: 233, GR: 250 }, ytdGross: 0, addlVacWeeks: 0 };
...
export default function Planner(){
  ...
  const dash = (() => {
    ...
    const futureVacWeeks = Math.round(futureVacDays / 7 * 10) / 10 + (settings.addlVacWeeks || 0);
    const totalVacWeeks = pastVacWeeks + futureVacWeeks;

    // Use manual YTD gross if provided, otherwise computed from planner
    const actualYtdGross = (settings.ytdGross && settings.ytdGross > 0) ? settings.ytdGross : ytdGross;
        ...
        <div className="dash-panel">
          <div className="dash-grid">
            <div className="dash-card">
              <div className="dash-label">YTD Gross{dash.usingManualGross?" (manual)":""}</div>
      ...
      {showSettings && (
        <div className="settings-panel">
          <div style={{padding:"12px 16px",display:"flex",flexWrap:"wrap",gap:16,alignItems:"flex-end"}}>
            ...
            <div>
              <label className="dash-label" style={{display:"block",marginBottom:4}}>YTD Gross ($)</label>
              <input type="number" className="tb-other" style={{width:100}} value={settings.ytdGross||""} placeholder="Auto"
                onChange={e=>setSettings(s=>({...s,ytdGross:parseInt(e.target.value)||0}))}/>
              <div style={{fontSize:9,color:"#999",marginTop:2}}>0 = compute from planner</div>
            </div>
            <div>
              <label className="dash-label" style={{display:"block",marginBottom:4}}>Addl Vacation (wks)</label>
              <input type="number" className="tb-other" style={{width:60}} value={settings.addlVacWeeks||0}
                onChange={e=>setSettings(s=>({...s,addlVacWeeks:parseFloat(e.target.value)||0}))}/>
              <div style={{fontSize:9,color:"#999",marginTop:2}}>Unscheduled future PTO</div>
            </div>
